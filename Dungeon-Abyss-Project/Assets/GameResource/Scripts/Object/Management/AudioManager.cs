...
namespace Backend.Object.Management
{
    public class AudioManager : SingletonGameObject<AudioManager>
    {
        ...
        private async UniTaskVoid PlaySfx_InternalAsync(string key, float pitch = 1f)
        {
            ...
            ReturnToPoolAfterPlay(audioSource, GetPitchedDuration(audioClip.length, pitch)).Forget();
        }
        ...
        private static float GetPitchedDuration(float clipLength, float pitch)
        {
            float safePitch = Mathf.Abs(pitch);
            if (safePitch < 0.01f)
                safePitch = 0.01f;
            return clipLength / safePitch;
        }
